...
          });
          ...
          // --- START FIX (This is the critical change) ---
          // This check prevents infinite loops by only setting a new
          // array reference if the data has actually changed.
          // We use JSON.stringify for a deep comparison.
          setData((prevData) => {
            ...
            if (prevDataString === newDataString) {
              return prevData; // Return the *old* stable array
            } else {
              return collectionData; // Return the *new* array
            }
          });
          // --- END FIX ---
...
export const addCalendarEvent = async (familyId, eventData) => {
  ...
  // Use the generic addFamilyDoc function, as it's a top-level collection
  // and we want the 'createdAt' timestamp.
  try {
    await addFamilyDoc(familyId, 'calendar', eventData);
  } catch (error) {
